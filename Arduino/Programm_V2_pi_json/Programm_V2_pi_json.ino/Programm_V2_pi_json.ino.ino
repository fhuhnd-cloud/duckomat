...
uint8_t goodBoxCapacity = 200;
uint8_t badBoxCapacity = 100;

bool invertDuckLogic = false;
...
bool isGoodDuckByQueue(uint8_t i) {
  bool hasRFID = qHasRFID(i);
  return invertDuckLogic ? !hasRFID : hasRFID;
}

bool isBadDuckByQueue(uint8_t i) {
  return !isGoodDuckByQueue(i);
}
...
void emitMachine(const char* eventName) {
  ...
  jsonKVNum(F("good_box_capacity"), (long)goodBoxCapacity);
  jsonKVNum(F("bad_box_capacity"), (long)badBoxCapacity);
  jsonKVBool(F("invert_duck_logic"), invertDuckLogic);
...
void processAutoDecision(uint8_t i) {
  bool goodDuck = isGoodDuckByQueue(i);
  bool badDuck = !goodDuck;
  ...
  if (goodDuck) {
    ...
    if (goodDucksCount >= goodBoxCapacity && currentBeltState == RUNNING) {
      setMotorSlow(0);
...
void handleServoStateMachine() {
  if (servoState == 1 && millis() - servoMoveTimer >= TIME_KICK) {
    ...
    if (badDucksCount >= badBoxCapacity) {
      badDucksCount = 0;
