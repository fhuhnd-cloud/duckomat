...
lock = threading.Lock()
started = False
runtime = {'mode':'IDLE','current_session_id':None,'current_mapping_mode':'idle','inventory_direction':'left','latest':{'boot':None,'sensors':{},'actuators':{},'machine':None,'rfid':None,'last_uid':None,'errors':[]}}
...
def belt_start(): return jsonify({'ok':send({'cmd':'actuator','name':'motor_main','state':1}) and send({'cmd':'actuator','name':'motor_feed','state':1})})
@APP.post('/api/hardware/belt/stop')
def belt_stop(): return jsonify({'ok':send({'cmd':'actuator','name':'motor_main','state':0}) and send({'cmd':'actuator','name':'motor_feed','state':0})})
...
def mapping_assign():
    payload = request.get_json(force=True); return jsonify(assign_mapping(payload['uid'], int(payload['duck_number']), bool(payload.get('overwrite', False))))
...
def inventory_start():
    payload = request.get_json(silent=True) or {}; expected_total = int(payload.get('expected_total', 0) or 0); runtime['mode'] = 'INVENTORY_RUNNING'
    with db() as c:
...
def inventory_pause(): runtime['mode'] = 'INVENTORY_PAUSED'; send({'cmd':'actuator','name':'motor_main','state':0}); send({'cmd':'actuator','name':'motor_feed','state':0}); return jsonify({'ok':True})
@APP.post('/api/inventory/resume')
def inventory_resume(): runtime['mode'] = 'INVENTORY_RUNNING'; send({'cmd':'actuator','name':'motor_main','state':1}); send({'cmd':'actuator','name':'motor_feed','state':1}); return jsonify({'ok':True})
...
def inventory_direction(): payload = request.get_json(force=True); runtime['inventory_direction'] = payload.get('direction', 'left'); return jsonify({'ok':True,'direction':runtime['inventory_direction']})
...
def inventory_summary():
    with db() as c:
        boxes = c.execute('SELECT box_name, count, capacity, state FROM box_state ORDER BY box_name').fetchall(); session = None; counts = {'mapped':0,'unmapped':0,'duplicate':0,'defective':0,'total':0,'expected_total':0}
