...
// ==========================================
#define PIN_LS 11       // Lichtschranke
  ...
  switch(currentState) {
    ...
    // ----------------------------------------------------
      ...
      if (Serial.available()) {
        ...
        if (antwort.indexOf("PASS") >= 0) {
          // WICHTIG: Warte, bis die Ente die Lichtschranke verlassen hat!
          while(digitalRead(PIN_LS) == LOW) { delay(10); }
          
          currentState = IDLE; 
        } 
        else if (antwort.indexOf("KICK_L") >= 0) {
          target_kick_pos = POS_UP_LEFT;
          if (current_pos != POS_UP_RIGHT) currentState = SWEEPING_SERVO;
          else { stateTimer = millis(); currentState = TIMING_KICK; }
        }
        else if (antwort.indexOf("KICK_R") >= 0) {
          target_kick_pos = POS_UP_RIGHT;
          if (current_pos != POS_UP_LEFT) currentState = SWEEPING_SERVO;
          else { stateTimer = millis(); currentState = TIMING_KICK; }
        }
      }
      else if (millis() - stateTimer > 500) {
        // Fallback: Kicken und Kicking-Timer starten
        target_kick_pos = POS_UP_LEFT;
        stateTimer = millis();
    ...
    // ----------------------------------------------------
      ...
      if (millis() - stateTimer > 300) { 
        // Kick ist physisch beendet.
        // Warte, bis die Ente die Lichtschranke komplett passiert hat:
        while(digitalRead(PIN_LS) == LOW) { delay(10); }
